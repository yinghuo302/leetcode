...
private:
    ...
    TweetNode* deep_copy(TweetNode* head){
        ...
        while(head){
            p->next=new TweetNode(head->id,head->time);p=p->next;
        }
        p=root->next;delete(root);
        return p;
    }
...
public:
    ...
    void postTweet(int userId, int tweetId) {
        if(users.empty()){
            ...
            users[userId]->post_tweet(tweetId,time);return ;
        }
    ...
    vector<int> getNewsFeed(int userId) {
        ...
        vector<int> res;TweetNode* h=r;
        while(res.size()<=10&&r){
            res.push_back(r->id);
            r=r->next;
        }
        deep_del(h);return res;
    }
...
vector<int> twoSum(vector<int>& nums, int target) {
    unordered_map<int,int> m;int size=nums.size();
...
int findMaxConsecutiveOnes(vector<int>& nums) {
    int size=nums.size();int res=0,count=0;
    for(int i=0;i<size;++i){
        if(nums[i])
            ++count;
        else if(count>res)
